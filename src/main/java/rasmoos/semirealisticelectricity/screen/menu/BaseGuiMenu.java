...
public abstract class BaseGuiMenu extends AbstractContainerMenu {
    ...
    public BaseGuiMenu(MenuType<?> menuType, int pContainerId, Inventory inv, FriendlyByteBuf extraData, ContainerData data) {
        this(menuType, pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), data);
    }

    public BaseGuiMenu(MenuType<?> menuType, int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(menuType, pContainerId);
        checkContainerSize(inv, IronFurnaceBlockEntity.NUM_SLOTS);

        if(!(entity instanceof BaseGuiBlockEntity)) {
            SemiRealisticElectricity.LOGGER.error("Trying to use a MachineMenu for a non MachineBlockEntity");
            throw new IllegalArgumentException();
        }

        this.entity = (BaseGuiBlockEntity) entity;
        this.level = inv.player.level;
        this.data = data;



        TE_INVENTORY_SLOT_COUNT = this.entity.getNumberOfSlots();
